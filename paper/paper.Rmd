...
<!-- 
The `contentid` package is a work in progress, but holds tremendous potential to provide portable, persistent data access. 
However, some issues still remain to be worked out. 

First, content identifiers are not well-linked to DOIs. 

--> 
<!--
I'd rather the task of going from contentid -> citation was considered out-of-scope for the package, and perhaps out-of-scope for any algorithm.  I feel citations are cultural -- just as one could never build a software package manager based on citation data (we agreed this was out of scope for codemeta), humans can and should be responsible to select what to cite.  contentid will give missing or 'incorrect' answers for some edge cases.

Second, I think this is more a solution looking for a problem than the reverse.
I agree that some bare R script with nothing but an opaque ID is a problem, but if anything still an improvement over the same script with a filename instead of a hash (since as your case demonstrates, a hash can be more definitively resolved to a DOI in some cases).
filenames meanwhile are terribly unreliable stores of metadata.
In the case that we write read_csv("vostok.co2"), no one is saying we need technology to figure out the citation for "vostok.co2", we already know how to do that part (and notably, it's not an algorithmic but a human process).
Scripts and datafiles can't exist naked in a vaccuum anyway, they've always needed metadata, such as a journal article and it's appendices and references, to understand how they are being used and how they were derived. No one first 'discovers' data by stumbling on a bare hash.

Third, the issue of the DOI, or the 'citation', is just a subset of the "second" issue outlined below, that of associating metadata to content.
-->



Second, content identifiers are **opaque**, and not particularly transparent.
<!-- Identifiers should be opaque, no? File names are poor stores for metadata... metadata belongs in metadata formats -->
For many researchers, seeing the long hash value in a script will not be very meaningful.
The `contentid` package needs mechanisms to transparently indicate what a content identifier refers to.
Again, we have this information available dynamically through the metadata collated in networks like [DataONE](https://dataone.org). 
Another useful extension to `contentid` would be to provide functions for displaying detailed metadata about a content identifier when it is available, as well as a mechanism to provide more human-readable aliases for the identifier.
<!-- 
There will always be hundreds of metadata formats, representing 100s of different fields.  Just about every one of those can use contentid-URIs to refer to the content they describe.  But I don't see why contentid has to pick
